-- MenuController.client.lua  v2.3
-- Ubicación Roblox: StarterGui  (LocalScript)
--
-- FIXES v2.3:
--   • buildGrid / buildLevelCard — tarjetas siempre reconstruidas con datos frescos
--   • Escucha LevelUnloaded Y ReturnToMenu (doble cobertura según versión de Boot)
--   • onReturnToMenu → va directo a S2 (selector), NO a S1
--   • loadProgress → reapplySelection() restaura el borde del nivel seleccionado
--     y llama updateSidebar() con datos recién llegados del servidor
...
local serverReadyEv   = remotesFolder and remotesFolder:WaitForChild("ServerReady",        5)
local requestPlayLEv  = remotesFolder and remotesFolder:WaitForChild("RequestPlayLevel",   5)
local levelReadyEv    = remotesFolder and remotesFolder:WaitForChild("LevelReady",         5)
local levelUnloadedEv = remotesFolder and remotesFolder:WaitForChild("LevelUnloaded",      5)  -- servidor→cliente
local returnToMenuEv  = remotesFolder and remotesFolder:FindFirstChild("ReturnToMenu")         -- no se usa en OnClientEvent normalmente
...
-- ── Fade (LoadingFrame dentro de GridArea) ─────────────────────────────────
...
local function buildLevelCard(lv, col, row, cont)
	...
	corner(10, imgZone)
	-- Tapar esquinas inferiores del clip
...
-- ── updateCardVisuals — actualiza una tarjeta ya existente ─────────────────
local function updateCardVisuals(card, lv)
	...
	if sbg then
		...
		local stxt=sbg:FindFirstChild("StatusText")
		if stxt then stxt.Text=STATUS_TEXTS[lv.status] or "—"; stxt.TextColor3=sc end
	end
	local nm=card:FindFirstChild("CardName",true)
	if nm then nm.TextColor3=lv.status=="bloqueado" and C.muted or C.text end
	local ss="" for i=1,3 do ss=ss..(i<=(lv.estrellas or 0) and "★" or "☆") end
...
-- ── buildGrid — destruye las tarjetas viejas y crea las nuevas ─────────────
local function buildGrid(progressData)
	...
	-- Eliminar contenido dinámico previo
	...
	-- Reconstruir LEVELS desde datos del servidor (claves STRING del payload)
	LEVELS = {}
	...
	-- Agrupar por campo "seccion" (fallback "NIVELES")
	local secciones = {}
	local ordenSec  = {}
	for i = 0, 4 do
	...
	-- ProgressBar=lo1, spacer=lo2, secciones desde lo3
	local lo   = 3
	local cols = 2
	...
	-- Ajustar CanvasSize al contenido real
...
-- ════════════════════════════════════════════════════════════════════════════
-- updateSidebar — panel derecho con detalles del nivel seleccionado
-- ════════════════════════════════════════════════════════════════════════════
updateSidebar = function(lv)
	...
	if hero then
		...
		if lv.imageId and lv.imageId ~= "" then
			...
			if not heroImage then
				heroImage=n("ImageLabel",{Name="HeroImage",Size=UDim2.new(1,0,1,0),
					BackgroundTransparency=1,Image=lv.imageId,
					ScaleType=Enum.ScaleType.Crop,ZIndex=6},hero)
			else
				heroImage.Image=lv.imageId; heroImage.Visible=true
			end
		else
	...
	local infoTag =infoBody:FindFirstChild("InfoTag");  if infoTag  then infoTag.Text =lv.tag        or "" end
	local infoName=infoBody:FindFirstChild("InfoName"); if infoName then infoName.Text=lv.nombre     or "" end
	local infoDesc=infoBody:FindFirstChild("InfoDesc"); if infoDesc then infoDesc.Text=lv.descripcion or "" end
	...
	if statsGrid then
		local function sv(nm, val)
			local box=statsGrid:FindFirstChild(nm); local lbl=box and box:FindFirstChild("Val")
	...
	if pb then
		if lv.status=="bloqueado" then
			...
			pb.Text=icon..(lv.nombre or ""):upper(); pb.TextColor3=C.black; pb.BackgroundColor3=C.accent3
...
-- ── connectLevelCards — conecta clicks a las tarjetas recién creadas ───────
	...
	for id=0,4 do
		...
		if card and card:IsA("TextButton") and LEVELS[id] then
			...
			card.MouseButton1Click:Connect(function()
				...
				-- Actualizar borde de todas las tarjetas
				for i=0,4 do
...
-- ── reapplySelection — restaura el borde resaltado tras reconstruir el grid ─
-- [FIX] Después de buildGrid las tarjetas son instancias nuevas; hay que
--       re-aplicar el highlight sin esperar un clic del usuario.
...
local function loadProgress()
	...
	-- [FIX] Re-resaltar la tarjeta seleccionada y actualizar sidebar con datos frescos.
	-- Esto es lo que corrige el bug: al volver de un nivel, buildGrid destruye y
	-- recrea las tarjetas; sin estos dos pasos la tarjeta aparece sin selección
	-- y la sidebar muestra datos viejos.
	reapplySelection()
...
-- Cerrar modales (CloseBtn, OkBtn, CancelBtn, SaveBtn)
for _, frame in ipairs({S3, S4, S5}) do
	for _, btnName in ipairs({"CloseBtn","OkBtn","CancelBtn","SaveBtn"}) do
		local b=frame:FindFirstChild(btnName,true)
	...
	playBtnSidebar.MouseButton1Click:Connect(function()
		...
		fadeIn(0.4, function()
			if requestPlayLEv then
				...
				-- Watchdog: 10 s sin respuesta del servidor
				task.spawn(function()
					task.wait(10)
...
-- ════════════════════════════════════════════════════════════════════════════
-- REGRESO AL MENÚ — cobertura dual
--
-- Boot puede disparar LevelUnloaded o ReturnToMenu dependiendo de la versión.
-- Ambos conectan a la misma función para no depender de cuál se usa.
--
-- Qué hace onReturnToMenu():
--   1. Habilita la GUI y va directo a S2 (selector), NO a S1 (menú principal)
--   2. Resetea progressLoaded para forzar recarga fresca del servidor
--   3. loadProgress() reconstruye tarjetas, re-resalta la selección y
--      actualiza la sidebar — así la tarjeta recién completada se ve
--      inmediatamente con su estado "COMPLETADO" sin clic adicional.
-- ════════════════════════════════════════════════════════════════════════════
local function onReturnToMenu()
	task.delay(0.1, function()
		-- Si el jugador ya inició otra carga (clic muy rápido en Jugar),
		-- solo marcar datos como obsoletos para refrescar en la próxima visita.
		-- NO tocar la cámara ni la GUI para no romper el nivel que está cargando.
		if isLoading then
			progressLoaded = false
			return
		end
		root.Enabled = true
		S1.Visible   = false
		S2.Visible   = true      -- ir directo al selector
		setupMenuCamera()
		UIS.MouseBehavior = Enum.MouseBehavior.Default
		-- selectedLevelID se conserva; loadProgress lo usa en reapplySelection
		-- y updateSidebar para mostrar datos del nivel recién terminado.
		progressLoaded = false
		task.spawn(loadProgress)
		print("[MenuController] Menú restaurado → selector de niveles")
	end)
end

if levelUnloadedEv then levelUnloadedEv.OnClientEvent:Connect(onReturnToMenu) end
if returnToMenuEv  then returnToMenuEv.OnClientEvent:Connect(onReturnToMenu)  end
...
-- Nombre del jugador en el top-bar
local playerTag=S2:FindFirstChild("PlayerTag",true)
if playerTag then playerTag.Text=player.DisplayName or player.Name end

setupMenuCamera()
print("[EDA v2] ✅ MenuController v2.3 — tarjetas dinámicas, actualización inmediata al regresar")
