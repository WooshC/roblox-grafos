...
function TimerEmergencia:iniciar(mision)
	...
	-- Loop de actualización cada segundo (usando task.wait en lugar de Heartbeat)
	self._conn = true  -- marca de que el timer está activo
	task.spawn(function()
		while self._conn do
			if not self._deadline or not self._jugador or not self._jugador.Parent then break end
